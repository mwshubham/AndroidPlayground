...
@Singleton
class CryptoSecurityRepositoryImpl
    @Inject
    constructor(
        ...
        override suspend fun fetchEncryptedFromServer(): SecureNetworkResponse = fakeApiService.fetchEncryptedResponse()
        ...
        override suspend fun saveKeystoreEncrypted(
            key: String,
            value: String,
        ) = keystoreDataStoreManager.encryptAndSave(key, value)

        override fun loadKeystoreDecrypted(key: String): Flow<String?> = keystoreDataStoreManager.loadAndDecrypt(key)

        override fun loadKeystoreRawIv(key: String): Flow<ByteArray?> = keystoreDataStoreManager.loadRawIv(key)

        override fun loadKeystoreRawCiphertext(key: String): Flow<ByteArray?> = keystoreDataStoreManager.loadRawCiphertext(key)

        override suspend fun clearKeystoreEntry(key: String) = keystoreDataStoreManager.clear(key)

        override suspend fun saveTinkEncrypted(
            key: String,
            value: String,
        ) = tinkDataStoreManager.encryptAndSave(key, value)

        override fun loadTinkDecrypted(key: String): Flow<String?> = tinkDataStoreManager.loadAndDecrypt(key)

        override fun loadTinkRawCiphertext(key: String): Flow<ByteArray?> = tinkDataStoreManager.loadRawCiphertext(key)

        override suspend fun clearTinkEntry(key: String) = tinkDataStoreManager.clear(key)

        override fun encryptForServer(plaintext: String): HybridEncryptedPayload = rsaCryptoManager.encryptForServer(plaintext.toByteArray(Charsets.UTF_8))

        override fun simulateServerDecrypt(payload: HybridEncryptedPayload): String = rsaCryptoManager.simulateServerDecrypt(payload).toString(Charsets.UTF_8)
    }
