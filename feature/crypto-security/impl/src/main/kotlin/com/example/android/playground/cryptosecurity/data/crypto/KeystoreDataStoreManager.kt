 ...
 */
@Singleton
class KeystoreDataStoreManager
    @Inject
    constructor(
        ...
        fun loadAndDecrypt(key: String): Flow<String?> =
            context.keystoreDataStore.data.map { prefs ->
                ...
                aesGcmCryptoManager
                    .decrypt(EncryptedData(iv = iv, ciphertext = ciphertext))
        ...
        fun loadRawIv(key: String): Flow<ByteArray?> = context.keystoreDataStore.data.map { prefs -> prefs[ivKey(key)] }

        /** Returns the raw ciphertext bytes for the given key, or null if not yet saved. */
        fun loadRawCiphertext(key: String): Flow<ByteArray?> = context.keystoreDataStore.data.map { prefs -> prefs[ciphertextKey(key)] }
