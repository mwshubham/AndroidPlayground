...
class SaveKeystoreEncryptedUseCaseTest {
    private val repository: CryptoSecurityRepository = mockk()
    private lateinit var useCase: SaveKeystoreEncryptedUseCase

    @Before
    fun setUp() {
        useCase = SaveKeystoreEncryptedUseCase(repository)
    }

    @Test
    fun `invoke delegates to repository with correct key and value`() =
        runTest {
            coEvery { repository.saveKeystoreEncrypted("myKey", "myValue") } just runs

            useCase("myKey", "myValue")

            coVerify(exactly = 1) { repository.saveKeystoreEncrypted("myKey", "myValue") }
        }

    @Test
    fun `invoke propagates exception from repository`() =
        runTest {
            coEvery { repository.saveKeystoreEncrypted(any(), any()) } throws RuntimeException("Keystore unavailable")
